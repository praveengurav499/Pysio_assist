...
export default function CameraPostureTracker({
          ...
          if (videoRef.current) {
            videoRef.current.play()
              .then(() => {
                ...
                // Trigger video scanning analysis loading phase
  ...
  const drawSkeleton = (ctx: CanvasRenderingContext2D, landmarks: any[], width: number, height: number) => {
    ...
    if (exerciseId === 'chintuck') {
      ...
      // Connect Head to Ears and Shoulders to Hips for a complete anatomical structure
      if (nose && activeEar) {
        drawAnimatedContrastLine(nose, activeEar, color);
      }
      if (nose && inactiveEar) {
        drawAnimatedContrastLine(nose, inactiveEar, color, true);
      }
      if (leftShoulder && leftHip) {
        drawAnimatedContrastLine(leftShoulder, leftHip, '#0ea5e9');
      }
      if (rightShoulder && rightHip) {
        drawAnimatedContrastLine(rightShoulder, rightHip, '#0ea5e9');
      }
      if (leftHip && rightHip) {
        drawAnimatedContrastLine(leftHip, rightHip, '#0ea5e9');
      }
    } else {
      ...
      // Connect Head & Face
      if (nose && leftEar) {
        drawAnimatedContrastLine(nose, leftEar, color);
      }
      if (nose && rightEar) {
        drawAnimatedContrastLine(nose, rightEar, color);
      }
      if (leftEar && leftShoulder) {
        drawAnimatedContrastLine(leftEar, leftShoulder, color);
      }
      if (rightEar && rightShoulder) {
        drawAnimatedContrastLine(rightEar, rightShoulder, color);
      }
      ...
      if (leftShoulder && rightShoulder) {
        ...
        if (leftHip && rightHip) {
          ...
          // Side flanks torso alignment lines
          drawAnimatedContrastLine(leftShoulder, leftHip, '#0ea5e9');
          drawAnimatedContrastLine(rightShoulder, rightHip, '#0ea5e9');
    ...
    const drawJoint = (pt: any, label: string, isA: boolean) => {
      ...
      // 5. Clean minimalist node design (no technical metadata labels overlapping the face)
      ctx.restore();
    };
    ...
    if (exerciseId === 'chintuck') {
      ...
      // Draw all Door Frame Stretch joints
      drawJoint(nose, 'NOSE', true);
  ...
  return (
    <div className="flex flex-col gap-4 w-full" id="camera-tracker-panel">
      ...
      <div className="relative aspect-video w-full bg-slate-950 border border-slate-800 overflow-hidden rounded-2xl shadow-inner flex items-center justify-center">
        ...
        {/* Analyzing Video Stream Overlay with cyber animations */}
        {isAnalyzingVideo && cameraActive && !isLoadingModel && !modelError && (
          <div className="absolute inset-0 bg-slate-950/95 z-30 flex flex-col items-center justify-center p-6 gap-4 text-center select-none font-mono">
            {/* Spinning radar ring */}
            <div className="relative w-24 h-24 flex items-center justify-center">
              <div className="absolute inset-0 rounded-full border-4 border-sky-500/15 animate-[spin_8s_linear_infinite]" />
              <div className="absolute inset-2 rounded-full border-4 border-dashed border-sky-400/40 animate-[spin_12s_linear_infinite]" />
              <div className="absolute inset-4 rounded-full border-2 border-emerald-500/30 animate-[ping_2s_linear_infinite]" />
              <Activity className="text-sky-400 animate-pulse stroke-[2.5]" size={36} />
              
              {/* Dynamic blinking scanner light dot */}
              <div className="absolute top-0 right-1/2 w-2 h-2 rounded-full bg-sky-400 shadow-[0_0_10px_#38bdf8] animate-bounce" />
            </div>

            <div className="space-y-1.5 max-w-sm">
              <span className="text-[10px] font-black tracking-widest text-sky-400 uppercase bg-sky-950/60 border border-sky-800/50 px-3 py-1 rounded-full animate-pulse">
                Analyzing Live Video Stream
              </span>
              <p className="text-xs font-bold text-slate-100">
                {analysisStep === 0 && "[SCANNING] Initializing video capture pipeline..."}
                {analysisStep === 1 && "[ANALYZING] Glare, exposure & ambient check..."}
                {analysisStep === 2 && "[CALIBRATING] Identifying joint center markers..."}
                {analysisStep === 3 && "[PIPELINE] Establishing cervical vector mesh..."}
                {analysisStep >= 4 && "[READY] Biomechanical neural mapping online!"}
              </p>
              <p className="text-[10px] text-slate-400 leading-normal">
                Align yourself centered in the frame. Deep learning posture engine starts shortly.
              </p>
            </div>

            {/* Simulated hardware scan bar */}
            <div className="w-48 h-1 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
              <div 
                ...
                style={{ width: `${Math.min(100, (analysisStep + 1) * 20)}%` }} 
              />
            </div>
          </div>
        )}
