...
export default function AIPostureAssistant({
  ...
  // Auto trigger analysis on initial mount, when exercise switches, or when detected posture stage changes
  const lastAnalyzedStageRef = useRef<'neutral' | 'correct' | 'bending' | 'forward' | null>(null);

  useEffect(() => {
    if (lastAnalyzedStageRef.current !== currentStage) {
      lastAnalyzedStageRef.current = currentStage;
      handleAnalyzePosture();
    }
  }, [activeExerciseId, currentStage]);
